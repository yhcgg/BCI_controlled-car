    ...
    while do_run:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN or label:
                ...
                if event.key == K_ESCAPE:
                    do_run = False
                elif event.key == K_UP or label == 13.0: 
                    window.fill((0., 0., 0.))
                    window.blit(rot_center(arrow, 180), (0, 0))
                elif event.key == K_DOWN or label == 17.0: 
                    window.fill((0., 0., 0.))
                    window.blit(arrow, (0, 0))
                elif event.key == K_RIGHT or label == 15.0:    
                    window.fill((0., 0., 0.))
                    window.blit(rot_center(arrow, 90), (0, 0))
                elif event.key == K_LEFT or label == 19.0:   
                    window.fill((0., 0., 0.))
                    window.blit(rot_center(arrow, 270), (0, 0))
                elif event.key == K_SPACE or label == 0.0:  
                    window.fill((0., 0., 0.))
                    window.blit(stop, (0, 0))
                label = None
...
def begin_car(code, freq, savename='', trials=1):
    ...
    # check whether eyes are open or close
    ...
    threshold = 0.5
    if (peak_alpha / peak_normal) < threshold:
        pred = 0    
    else:
        pred = 1   
    print("pred: ", pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument("port_num", type=str, help="Arduino bluetooth serial port")     # input car's port
    args = parser.parse_args()

    ser = serial.Serial(args.port_num, 9600, timeout=1, write_timeout=1)

    # run 2 rounds to move 1 step
    code = []
    for i in range(2):
         code.append(begin_car(i, 1, i))
    
    if code == [0,1]:       # forward: eyes open --> closed
        ser.write(b'1') 
    elif code == [0, 0]:    # right: eyes open --> open
        ser.write(b'4') 
    elif code == [1, 0]:    # left: eyes close --> open
        ser.write(b'3') 
